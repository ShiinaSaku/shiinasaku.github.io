...
function toColorArray(stops: string[]) {
  return stops.map((hex: string) => {
...
function renderFrame(time: number) {
  ...
  program.uniforms.uColorStops.value = toColorArray(props.colorStops);
...
watch(
  ...
  () => {
    if (!props.animate) renderFrame(props.time ?? 8);
  },
  { deep: true },
);
...
onMounted(() => {
  ...
  program = new Program(gl, {
    ...
    uniforms: {
      ...
      uColorStops: { value: toColorArray(props.colorStops) },
  ...
  if (props.animate) {
    const update = (t: number) => {
      ...
      renderFrame(props.time ?? t * 0.01);
    };
    animateId = requestAnimationFrame(update);
  } else {
    renderFrame(props.time ?? 8);
  }
