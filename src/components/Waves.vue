...
function themeLineColor(): string {
  return document.documentElement.classList.contains("dark")
    ? "rgba(247, 81, 146, 0.4)"
    : "rgba(147, 104, 181, 0.45)";
}
...
let config: Config = {
  lineColor: props.lineColor ?? "rgba(147, 104, 181, 0.45)",
...
onMounted(() => {
  ...
  if (!props.lineColor) config.lineColor = themeLineColor();
  themeObserver = new MutationObserver(() => {
    if (!props.lineColor) {
      config.lineColor = themeLineColor();
      if (prefersReducedMotion) drawStaticFrame();
    }
  });
...
watch(
  ...
  () => {
    config = {
      lineColor: props.lineColor ?? themeLineColor(),
      waveSpeedX: props.waveSpeedX,
      waveSpeedY: props.waveSpeedY,
      waveAmpX: props.waveAmpX,
      waveAmpY: props.waveAmpY,
